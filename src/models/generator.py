import tensorflow as tf

import util.ops as ops
import util.utils as utils


class Generator:
    """
    Generator for FP-GAN
    """
    def __init__(self, name, is_training, ngf=64, norm='instance',
                 image_size=None, rgb=True):
        """

        Args:
            name: will be used as scope
            is_training: boolean
            ngf: number of filters in first layer
            norm: instance or batch
            image_size: (width, height)
            rgb: color or gray-scale images
        """
        self.name = name
        self.reuse = False
        self.ngf = ngf
        self.norm = norm
        self.is_training = is_training
        self.image_size = image_size
        self.rgb = rgb

    def __call__(self, input):
        """
        Args:
          input: batch_size x width x height x 3
        Returns:
          output: same size as input
        """
        with tf.variable_scope(self.name):
            # conv layers
            c7s1_32 = ops.c7s1_k(input, self.ngf, is_training=self.is_training,
                                 norm=self.norm,
                                 reuse=self.reuse,
                                 name='c7s1_32')  # (?, w, h, 32)
            d64 = ops.dk(c7s1_32, 2 * self.ngf, is_training=self.is_training,
                         norm=self.norm,
                         reuse=self.reuse, name='d64')  # (?, w/2, h/2, 64)
            d128 = ops.dk(d64, 4 * self.ngf, is_training=self.is_training,
                          norm=self.norm,
                          reuse=self.reuse, name='d128')  # (?, w/4, h/4, 128)

            if self.image_size[0] <= 128 and self.image_size[1] <= 128:
                # use 6 residual blocks for 128x128 images
                res_output = ops.n_res_blocks(d128, reuse=self.reuse,
                                              n=6)  # (?, w/4, h/4, 128)
            else:
                # 9 blocks for higher resolution
                res_output = ops.n_res_blocks(d128, reuse=self.reuse,
                                              n=9)  # (?, w/4, h/4, 128)

            # fractional-strided convolution
            u64 = ops.uk(res_output, 2 * self.ngf,
                         is_training=self.is_training, norm=self.norm,
                         reuse=self.reuse, name='u64')  # (?, w/2, h/2, 64)
            u32 = ops.uk(u64, self.ngf, is_training=self.is_training,
                         norm=self.norm,
                         reuse=self.reuse, name='u32',
                         output_size=self.image_size)  # (?, w, h, 32)

            n_output_channels = input.shape[3]  # This can be 1 or 3
            # conv layer
            # Note: the paper said that ReLU and _norm were used
            # but actually tanh was used and no _norm here
            output = ops.c7s1_k(u32, n_output_channels, norm=None,
                                activation='tanh', reuse=self.reuse,
                                name='output')  # (?, w, h, 3)
        # set reuse=True for next call
        self.reuse = True
        self.variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                           scope=self.name)

        return output

    def sample(self, input):
        """
        Produces a sample output for given input.
        The output is encoded as jpeg tensor.
        Args:
            input: tensor

        Returns:

        """
        images = utils.batch_convert2int(self.__call__(input))
        images_jpeg = tf.map_fn(lambda img: tf.image.encode_jpeg(img), images,
                                dtype=tf.string)
        return images_jpeg
